/**
 * 품목별 매출 분석 (분석 리포트 · 재설계)
 * ⚠️ 레거시 원본 불일치(메뉴명 ≠ 실제 데이터소스, 로컬 스펙 문서 참조,
...
export default async function SalesByItemPage({
  ...
  return (
    <div>
      <PageHeader
        title="품목별 매출 분석"
        description="품목별 매출 집계 리포트 — OINV/INV1 기반(신규 설계, 로컬 스펙 문서 참조)"
      />

      <SearchBar
        fields={SEARCH_FIELDS}
        values={{ sItemCode, searchFrom, searchTo }}
        storageKey={`b1w:search:sales-by-item:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<ItemSalesRow & { pct: number }>
            ...
            storageKey={`b1w:cols:sales-by-item:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
