/**
 * 공급처별 매입 분석 — 거래처별 매출 분석의 구매측 미러(GROUP BY 집계 리포트).
 *
 * 차원: 공급처(OPCH.CardCode) / 측정값: 전표건수(COUNT), 매입액(SUM DocTotal) / 비중%(매입액/전체합*100)
 * 원천: OPCH(AP송장 헤더), CANCELED='N'. 정렬: 매입액 DESC. 합계행: 전체 건수·매입액.
 */
...
const COLUMNS: Column<VendorPurchaseRow>[] = [
  { key: "CardCode", header: "공급처코드", width: "120px" },
  { key: "CardName", header: "공급처명" },
  {
    key: "DocCount",
    header: "전표건수",
    align: "right",
  ...
  },
  {
    ...
    align: "right",
    width: "150px",
    render: (r) => formatMoney(r.PurchAmount),
  },
  {
    key: "Ratio",
    header: "비중",
...
const SEARCH_FIELDS: SearchFieldDef[] = [
  { name: "sCardCode", label: "공급처", kind: "cfl", cflType: "Vendor", placeholder: "공급처 선택" },
  { name: "searchFrom", label: "전기일(시작)", kind: "date" },
  { name: "searchTo", label: "전기일(종료)", kind: "date" },
];

export default async function PurchaseByVendorPage({
  searchParams,
}: {
  ...
  try {
    ...
    const all = await query<{
      ...
      PurchAmount: string | number;
      GrandTotal: string | number;
    }>(
      `SELECT TOP ${MAX_DISPLAY} i."CardCode", i."CardName",
              COUNT(*) AS "DocCount",
              SUM(TO_DECIMAL(i."DocTotal", 19, 2)) AS "PurchAmount",
  ...
  return (
    <div>
      <PageHeader
        title="공급처별 매입 분석"
        description="공급처별 매입 집계 — OPCH 기준(매출 분석 미러)"
      />

      <SearchBar
        fields={SEARCH_FIELDS}
        values={{ sCardCode, searchFrom, searchTo }}
        storageKey={`b1w:search:purchase-by-vendor:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          <p className="mb-2 text-sm text-muted">
            {rows.length.toLocaleString()}개 공급처{rows.length >= MAX_DISPLAY ? ` (상위 ${MAX_DISPLAY}건만 표시)` : ""}
          </p>
          <DataGrid<VendorPurchaseRow>
            columns={COLUMNS}
            rows={rows}
            getRowKey={(r) => r.CardCode}
            emptyText="조건에 맞는 매입 데이터가 없습니다."
            storageKey={`b1w:cols:purchase-by-vendor:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
