/**
 * W3100 — 일일현황 (분석 리포트 재설계)
 * ⚠️ 역설계 확정: 원본에 '/W3100'='일일현황' 메뉴는 있으나,
 * 레거시 원본 소스 전체를 검색한 결과 이 URL을 처리하는 Spring 컨트롤러가 존재하지 않음
 * (인접 메뉴 W3060/W3070/W3080/W3090도 동일 — 리포트 그룹 전반이 메뉴명≠실제기능 상태).
 * 레거시 원본 재현이 불가능한 dead menu이므로, 메뉴명 '일일현황' 의도에 맞춰
 * 문서목록이 아닌 **집계형 분석 리포트**로 재설계함(사용자 승인 2026-07-06, rules.md
 * "분석 리포트 화면(집계형)" 섹션 정책 적용).
 *
 ...
 * (docs/conversion/specs/W3100.json uncertainties 참조 — 역설계 확정사항 + 검증 실측치)
 */
...
export default async function W3100Page({
  searchParams,
}: {
  ...
  return (
    <div>
      <PageHeader
        title="일일현황"
        description="W3100 · 분석 리포트(재설계) — 원본 미구현 메뉴(컨트롤러 없음, 404) 확인됨. 일자별 주문·납품·매출 크로스 요약"
      />

      <SearchBar
        fields={SEARCH_FIELDS}
        values={{ searchFrom, searchTo }}
        storageKey={`b1w:search:W3100:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<DailySummaryRow>
            ...
            emptyText="조건에 맞는 일일현황 데이터가 없습니다."
            storageKey={`b1w:cols:W3100:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
