...
/** 판매주문 폼 참조데이터(지점·창고·코스트센터) 로더 — new/edit 페이지 공용.
...
export async function loadOrderRefs(): Promise<{
  branches: Opt[];
  warehouses: Opt[];
  costCenters: Opt[];
  currencies: Opt[];
  ...
  const [branches, warehouses, costCenters, currencies, vatGroups, paymentTerms, headerUdfs, lineUdfs] = await Promise.all([
    ...
    // 매출 세금그룹(OVTG Category='O') + 세율. A8(과세-세금계산서) 최상단 고정
    query<{ Code: string; Name: string; Rate: number }>(
      `SELECT "Code", "Name", "Rate" FROM OVTG
       WHERE "Category"='O' AND ("Inactive" IS NULL OR "Inactive"='N')
       ORDER BY CASE WHEN "Code"='A8' THEN 0 ELSE 1 END, "Code"`,
    ),
    // 지불조건 (OCTG) — 상세 탭
    query<{ GroupNum: number; PymntGroup: string }>(
      `SELECT "GroupNum", "PymntGroup" FROM OCTG WHERE "GroupNum" > 0 ORDER BY "GroupNum"`,
    ),
    // UDF 정의 — 헤더(ORDR)/라인(RDR1). 생성폼이 상세 선택대로 입력칸 노출
    loadTableUdfs("ORDR"),
    loadTableUdfs("RDR1"),
  ]);
  return {
