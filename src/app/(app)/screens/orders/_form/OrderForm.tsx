...
import { saveOrder, cancelOrder } from "./actions";
import type { Opt, VatOpt, OrderInitial, OrderLineValue, SaveOrderResult } from "./types";
import { useLineColVisible, useHeaderFieldVisible, UdfInput, type UdfDefC } from "../../_shared/docFormLayout";

let uid = 1;
const emptyLine = (whs: string, cc: string, vat: string, ship: string): OrderLineValue => ({
  key: `n${uid++}`,
  itemCode: "",
  itemName: "",
  quantity: "0",
...
export default function OrderForm({
  ...
  /** 상세 컬럼선택 저장키 네임스페이스(예 "orders") — 생성폼이 그 선택대로 입력칸 노출 */
  storageNs: string;
  /** 사용자 키(상세와 동일 저장키 공유) */
  uid: string;
}) {
  ...
  const [result, setResult] = useState<SaveOrderResult | null>(null);
  const [cancelConfirm, setCancelConfirm] = useState(false);
  const isEdit = mode === "edit";

  const defWhs = refs.warehouses.find((w) => w.value === "W1000")?.value ?? refs.warehouses[0]?.value ?? "";
  const defCc = refs.costCenters.find((c) => c.value === "210001")?.value ?? refs.costCenters[0]?.value ?? "";
  const defVat = refs.vatGroups.find((v) => v.value === "A8")?.value ?? refs.vatGroups[0]?.value ?? "";
  ...
  const setLine = (key: string, patch: Partial<OrderLineValue>) =>
  ...
  const vatOf = (l: OrderLineValue) => Math.round((supplyOf(l) * rateOf(l.vatGroup)) / 100);
  ...
  const submit = () => {
    ...
    start(async () => {
      ...
      setResult(r);
      if (r.ok) {
        router.push(`/screens/orders/${r.docEntry}`);
        router.refresh();
      }
    });
  };
  ...
  const doCancel = () =>
    startCancel(async () => {
      ...
      const r = await cancelOrder(docEntry);
      if (r.ok) {
        router.push(`/screens/orders/${docEntry}`);
        router.refresh();
      } else {
  ...
  const actions = (
    <>
      ...
      {isEdit ? (
        <Link href="/screens/orders/new" className={buttonVariants({ variant: "outline", size: "sm" })}>
          신규
        </Link>
      ) : (
      ...
      <Link
        href={isEdit && docEntry ? `/screens/orders/${docEntry}` : "/screens/orders"}
  ...
  return (
    <div className="flex flex-col gap-4">
      <DocumentFormShell
        title={isEdit ? `판매오더 #${initial.docNum || docEntry}` : "판매오더 신규"}
        status={
