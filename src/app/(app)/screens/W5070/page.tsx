...
export default async function W5070Page({
  searchParams,
}: {
  ...
  try {
    // CF_W5000(Gubun,SDT,EDT,ITEMCD,WHSCD,TYPE,CARDCD,ETC) — Gubun/SDT/TYPE/CARDCD/ETC 서버 고정
    const all = await callProc<StockRow>("CF_W5000", ["W", "", edt, item, whs, "", "", ""]);
  ...
  return (
    <div>
      ...
      <SearchBar
        ...
        values={{ edt, whs, item }}
        storageKey={`b1w:search:W5070:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<StockRow>
            ...
            storageKey={`b1w:cols:W5070:${user?.uid ?? "anon"}`}
          />
        </>
      )}
    </div>
