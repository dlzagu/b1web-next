 ...
 *    화면 이름에 맞게 신설했다. (docs/conversion/specs/W3071.json uncertainties 참조)
 *
 ...
 * 패턴: W5071(DataGrid group/frozen/total) + items/[itemCode](그리드 세로 스택) 복제.
 */
import {
  PageHeader,
  SearchBar,
...
${lineWhere}
 ...
 LIMIT ${MAX_LINES}`,
  ...
  const hrefForPartner = (cardCode: string) => {
    ...
    return `/screens/W3071?${p.toString()}`;
  };
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<PartnerRow>
            ...
            storageKey={`b1w:cols:W3071:partner:${user?.uid ?? "anon"}`}
          />
          ...
          <DataGrid<LineRow>
            ...
            storageKey={`b1w:cols:W3071:line:${user?.uid ?? "anon"}`}
          />
        </>
      )}
    </div>
