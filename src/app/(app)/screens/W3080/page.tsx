/**
 * W3080 — 매출 누계 추이 (분석 리포트 화면 · GROUP BY 집계)
 * 🚩 레거시 소스 전체에 `/W3080` 컨트롤러가 없음(완전한 죽은 메뉴 -- W3060/70/90도 동일).
 ...
 * (docs/conversion/specs/W3080.json uncertainties 참조 — 역설계 확정사항)
 */
...
export default async function W3080Page({
  searchParams,
}: {
  ...
  // 거래처 select 옵션 — OCRD 영업 거래처만 (CardType='C' AND validFor='Y'), W3030 패턴 그대로
  ...
  return (
    <div>
      <PageHeader
        title="매출 누계 추이"
        description="W3080 · 분석 리포트(집계형) — 레거시 컨트롤러 부재(죽은 메뉴), OINV 월별 매출 GROUP BY 집계 + 누적합으로 신규 설계"
      />

      <SearchBar
        fields={searchFields}
        values={{ sCardCode, searchFrom, searchTo }}
        storageKey={`b1w:search:W3080:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<TrendRow>
            ...
            storageKey={`b1w:cols:W3080:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
