...
/**
 * 매출 세금계산서 상세 액션 — [취소](확인 2단계). cancelInvoice 서버액션(상태가드) → 성공 시 router.refresh.
 * editable=false(취소/마감)면 아무것도 노출하지 않음. (목록 링크는 DocBaseDetail 액션바가 추가)
 */
import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { Button } from "@/components/ui";
import { cancelInvoice } from "../_form/actions";

export function InvoiceActions({ docEntry, editable }: { docEntry: number; editable: boolean }) {
  ...
  const doCancel = () =>
    start(async () => {
      setErr(null);
      const r = await cancelInvoice(docEntry);
      if (r.ok) {
        setConfirming(false);
        router.refresh();
      } else {
