 ...
 * 판매 리포트 그룹의 데이터소스는 W3030 검증에서 OINV로 확정된 바 준용.
 * (docs/conversion/specs/W3060.json uncertainties 참조 — 역설계 확정사항)
 * 패턴: W3030/page.tsx 복제 (force-dynamic, 동적 WHERE 조립 + 파라미터 바인딩, GROUP BY 집계형 + 비중%).
 */
...
type SalesByPeriodDisplayRow = SalesByPeriodRow & { Ratio: number };
...
const COLUMNS: Column<SalesByPeriodDisplayRow>[] = [
  { key: "YM", header: "기간(월)", align: "center", width: "120px" },
  { key: "DocCnt", header: "전표건수", align: "right", width: "110px", render: (r) => formatNumber(r.DocCnt) },
  { key: "SumTotal", header: "매출액", align: "right", width: "170px", render: (r) => formatMoney(r.SumTotal) },
...
export default async function W3060Page({
  searchParams,
}: {
  ...
  let rows: SalesByPeriodDisplayRow[] = [];
  let totalCnt = 0;
  let totalSum = 0;
  ...
  try {
    ...
    const raw = await query<SalesByPeriodRow>(
  ...
  return (
    <div>
      ...
      <SearchBar
        ...
        storageKey={`b1w:search:W3060:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<SalesByPeriodDisplayRow>
            columns={COLUMNS}
            rows={rows}
            getRowKey={(r) => r.YM}
            emptyText="조건에 맞는 판매 데이터가 없습니다."
            storageKey={`b1w:cols:W3060:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
