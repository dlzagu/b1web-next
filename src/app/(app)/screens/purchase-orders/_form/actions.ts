...
/**
 * 구매주문 저장/취소 서버 액션 — 생성·수정·취소 오케스트레이션. 판매주문 미러.
 * 쓰기는 SL 경유만(documents.ts). 상태 가드(취소/마감 문서 차단)는 여기서 HANA 읽기로 선검증.
 * 함정 회피: 비멱등 POST 재시도 금지(client), 취소 전 상태검증.
 */
import {
  createPurchaseOrder as slCreatePurchaseOrder,
  updatePurchaseOrder as slUpdatePurchaseOrder,
  cancelPurchaseOrder as slCancelPurchaseOrder,
} from "@/lib/sl/documents";
...
type PurchaseOrderStatus = { CANCELED: string; DocStatus: string };

async function getPurchaseOrderStatus(docEntry: number): Promise<PurchaseOrderStatus | null> {
  const rows = await query<PurchaseOrderStatus>(
    `SELECT "CANCELED", "DocStatus" FROM "OPOR" WHERE "DocEntry" = ?`,
    [docEntry],
  );
  return rows[0] ?? null;
}
...
function editBlockReason(st: PurchaseOrderStatus | null): string | null {
...
export async function savePurchaseOrder(p: SavePurchaseOrderPayload): Promise<SavePurchaseOrderResult> {
  ...
  if (lines.length === 0) return { ok: false, error: "유효한 구매오더 라인(품목+수량)이 없습니다." };

  try {
    if (p.docEntry != null) {
      // 수정 — 상태 가드 후 PATCH(전체 라인 교체)
      const reason = editBlockReason(await getPurchaseOrderStatus(p.docEntry));
      if (reason) return { ok: false, error: reason };
      await slUpdatePurchaseOrder({
        DocEntry: p.docEntry,
    ...
    // 생성
    const res = await slCreatePurchaseOrder({
      CardCode: p.cardCode,
...
export async function cancelPurchaseOrder(docEntry: number): Promise<CancelPurchaseOrderResult> {
  if (!(await getSession())) return { ok: false, error: "세션이 만료되었습니다. 다시 로그인하세요." };
  try {
    const reason = editBlockReason(await getPurchaseOrderStatus(docEntry));
    if (reason) return { ok: false, error: reason };
    await slCancelPurchaseOrder(docEntry);
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : String(e) };
  }
