/**
 * 구매주문 수정 — 상세([docEntry])의 [수정] 진입점. 판매주문 미러.
 * OPOR 헤더 + POR1 라인을 로드해 PurchaseOrderForm(edit 모드)에 프리필. 취소/마감 문서는 편집 차단.
...
import PurchaseOrderForm from "../../_form/PurchaseOrderForm";
import { loadPurchaseOrderRefs } from "../../_form/refs";
import type { PurchaseOrderLineValue } from "../../_form/types";
...
export default async function EditPurchaseOrderPage({ params }: { params: Promise<{ docEntry: string }> }) {
  ...
  let header: (PurchaseOrderHeader & Record<string, unknown>) | null = null;
  let lines: (LineRow & Record<string, unknown>)[] = [];
  let refs: Awaited<ReturnType<typeof loadPurchaseOrderRefs>> | null = null;
  let error: string | null = null;

  try {
    refs = await loadPurchaseOrderRefs();
    const hs = await query<PurchaseOrderHeader & Record<string, unknown>>(
    ...
    if (header) {
      lines = await query<LineRow & Record<string, unknown>>(
        `SELECT l."LineNum", l."ItemCode", l."Dscription", l."Quantity", l."Price",
         ...
         FROM "POR1" l WHERE l."DocEntry" = ? ORDER BY l."LineNum"`,
        [id],
      );
    }
  } catch (e) {
    ...
    <div className="mx-auto max-w-6xl">
      {error ? (
        ...
        <div className="rounded-card border border-warning/30 bg-warning-bg p-4 text-sm text-warning-fg">
          ...
          <Link href={`/screens/purchase-orders/${id}`} className="font-medium underline">
            상세로 돌아가기
          </Link>
        </div>
      ) : header && refs ? (
