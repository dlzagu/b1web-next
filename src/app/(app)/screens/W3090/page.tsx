/**
 * W3090 — 거래처 매출 누계 (분석 리포트 재설계)
 *
 * ⚠️ 레거시 컨트롤러 불일치(docs/conversion/specs/W3090.json uncertainties 참조):
 * 원본 메뉴명은 '거래처 매출 누계'이지만 실제 /W3090 컨트롤러는 전혀 다른 용도
...
export default async function W3090Page({
  searchParams,
}: {
  ...
  return (
    <div>
      <PageHeader
        title="거래처 매출 누계"
        description="W3090 · 거래처 매출 누계 분석 — OINV 기반 GROUP BY 집계(레거시 컨트롤러 불일치로 메뉴명 기준 재설계, spec 참조)"
      />

      <SearchBar
        fields={SEARCH_FIELDS}
        values={{ sCardCode, searchFrom, searchTo }}
        storageKey={`b1w:search:W3090:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<CustomerSalesRow>
            ...
            storageKey={`b1w:cols:W3090:${user?.uid ?? "anon"}`}
          />
          {rows.length > 0 && (
