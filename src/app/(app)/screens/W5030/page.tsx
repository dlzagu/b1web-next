 ...
 * 패턴: W3030/screens/W3030/page.tsx 복제 (force-dynamic, 동적 WHERE 조립 + 파라미터 바인딩).
 */
...
export default async function W5030Page({
  searchParams,
}: {
  ...
  // 거래처 select 옵션 — OCRD 영업 거래처만 (CardType='C' AND validFor='Y', W3030과 동일 가정)
  ...
  return (
    <div>
      ...
      <SearchBar
        ...
        storageKey={`b1w:search:W5030:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<TransferRequestRow>
            ...
            rowHref={(r) => `/screens/W5030/${r.DocEntry}`}
            emptyText="조건에 맞는 재고이전요청이 없습니다."
            storageKey={`b1w:cols:W5030:${user?.uid ?? "anon"}`}
          />
        </>
      )}
    </div>
