 ...
 * 패턴: 재고 이동 내역 / 월별 매출 추이 복제 + DataGrid 피벗 옵션(group/frozen/total) 사용.
 */
import {
  PageHeader,
  SearchBar,
...
LIMIT ${MAX_DISPLAY}`;
  ...
  const hrefFor = (nextTab: string) => {
    ...
    return `/screens/stock-ledger?${p.toString()}`;
  };
  ...
  return (
    <div>
      <PageHeader title="재고 입출고 대장" description="OINM 원장 집계 (수량 기준)" />
      <div className="mb-3">
      ...
      <SearchBar
        ...
        storageKey={`b1w:search:stock-ledger:${user?.uid ?? "anon"}`}
      />
      ...
      {error ? (
        ...
        <>
          ...
          <DataGrid<Row>
            ...
            storageKey={`b1w:cols:stock-ledger:${tab}:${user?.uid ?? "anon"}`}
          />
        </>
      )}
    </div>
