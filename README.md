...
| **서버리스에서 쓰기 영속** | 인스턴스마다 DB가 따로 뜨는 문제를 공유 DB(libSQL/Turso)로 해결. **엔진 코드는 한 줄도 안 고치고** 드라이버만 교체했고, 그 대신 커스텀 SQL 함수·트랜잭션 중첩이 막히는 것을 SQL 재작성과 SAVEPOINT로 넘겼다 ([ADR-0002](docs/decisions/ADR-0002-shared-db-turso.md)) |
...
- **`AUTH_SECRET`** — 세션 쿠키의 HMAC 서명 키. **이 데모는 일부러 설정하지 않았다**(미설정 시
  저장소에 공개된 기본값으로 폴백). 즉 쿠키 위조가 가능한데, 그래도 되는 이유는
  **위조로 더 얻을 수 있는 것이 없기 때문**이다 — 로그인 화면의 [데모 계정으로 둘러보기]
  버튼이 이미 누구에게나 세션을 내주고, 계정이 하나뿐이라 권한·역할 구분도 없다.
  공유 DB의 쓰기를 지키는 것도 이 키가 아니라 **매일 초기화**다.
  **실사용자 인증이나 권한 분리를 붙이는 순간 이 키가 유일한 방어선이 되므로**, 그때는
  32바이트 랜덤값을 환경변수로 넣어야 한다
