...
- **문서** (6) — 판매오더 / 납품 / 매출 세금계산서 · 구매오더 / 구매입고 / 매입 세금계산서 (목록·상세·생성·수정·취소)
...
```bash
npm run verify     # lint → typecheck → smoke 47항목
```
...
| 9. 리포트 대사 | **문서를 실제로 취소시킨 뒤** 공급처축·월축이 같은 금액만큼 함께 줄어드는가 · 재고 피벗 합 = 원본 합 |
